#!/usr/bin/env node
/*
  Downsample time,price CSV from 1-minute to 10-minute buckets.
  Usage:
    node scripts/downsample-10min.js [inputCSV] [outputCSV]
  Defaults:
    inputCSV = data/bootstrap.csv
    outputCSV = data/bootstrap_10min.csv
*/
const fs = require('fs');
...
function floorTo10MinUTC(date) {
  ...
  const floored = Math.floor(m / 10) * 10;
...
async function downsample(inputPath, outputPath) {
  ...
  for await (const line of rl) {
    ...
    const bucket = floorTo10MinUTC(date).toISOString();
...
(async () => {
  ...
  const output = process.argv[3] || path.join('data', 'bootstrap_10min.csv');
